#!/usr/bin/env python3
"""
Pre-compute RadarXFormer-Inspired features V3 for NuScenes VQA.

V3 UPGRADE: Uses YOLOWorld (open-vocabulary) instead of COCO YOLOv8m.
This detects ALL 23 NuScenes categories including trailers, construction
vehicles, ambulances, police cars, barriers, traffic cones — categories
that COCO-trained YOLO completely misses.

Output shape: (MAX_OBJECTS, 48) per sample
  Dims  0-15: Enhanced structured features (same layout as annotation features)
  Dims 16-47: CLIP visual features (PCA-compressed from 512-d)
...
Usage:
    ...
    CUDA_VISIBLE_DEVICES=0 python precompute_radarxformer_features_v3.py --mode all
"""
...
OUT_DIR = "/media/nas_mount/anwar2/experiment/dataset/nuscenes/nischay/radarxf_features_v3"
YOLO_MODEL = "yolov8x-worldv2.pt"   # YOLOWorld open-vocabulary
CONF_THRESHOLD = 0.15
MAX_OBJECTS = 100
STRUCT_DIM = 16
CLIP_RAW_DIM = 512
...
# PCA model path
PCA_MODEL_PATH = os.path.join(os.path.dirname(__file__), "clip_pca_model_v3.pkl")


# ============================================================
# YOLOWorld NuScenes Classes (ALL 23 categories)
# ============================================================

YOLOWORLD_CLASSES = [
    # (text_prompt, nuscenes_category_id, is_vehicle)
    ("adult person walking",        0, False),
    ("child",                       1, False),
    ("person in wheelchair",        2, False),
    ("baby stroller",               3, False),
    ("person on scooter",           4, False),
    ("police officer",              5, False),
    ("construction worker",         6, False),
    ("animal",                      7, False),
    ("car",                         8, True),
    ("motorcycle",                  9, True),
    ("bicycle",                    10, False),
    ("articulated bus",            11, True),
    ("bus",                        12, True),
    ("truck",                      13, True),
    ("construction vehicle",       14, True),
    ("ambulance",                  15, True),
    ("police car",                 16, True),
    ("trailer",                    17, True),
    ("road barrier",               18, False),
    ("traffic cone",               19, False),
    ("trash can",                  20, False),
    ("debris on road",             21, False),
    ("bicycle rack",               22, False),
]
...
VALID_YOLO_IDS = set(range(len(YOLOWORLD_CLASSES)))
...
def extract_sample_features(nusc, sample_token, yolo_model, clip_extractor,
    ...
    for cam_ch in CAMERA_CHANNELS:
        ...
        # Filter to valid NuScenes classes (YOLOWorld: all indices are valid)
        ...
        for j in range(len(valid_indices)):
            yolo_cls = int(valid_classes[j])
            nusc_cat_id, is_vehicle = PROMPT_TO_NUSCENES[yolo_cls]
...
def main():
    ...
    print(f"Loading YOLOWorld model: {args.yolo_model}")
    yolo = YOLOWorld(args.yolo_model)
    yolo.to(args.device)
    yolo.set_classes(PROMPT_LIST)
    print(f"  Set {len(PROMPT_LIST)} NuScenes class prompts")
